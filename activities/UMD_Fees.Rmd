...
```{r}
umd_fees_pct_25 <- umd_fees |> mutate(pct_change = (`Fall 2025`- `Fall 2024`)/`Fall 2024` * 100)
```
...
Overall, UMD fees have increased between 2021 and 2025. But not evenly. Some fees, like the Athletic Fee, remained consistent. Others, like the Shuttle Bus fee, increased significantly. Additionally, there was no fee for the Counseling Center in 2021, so we can only compare increases for that fee from 2022 onward. 
