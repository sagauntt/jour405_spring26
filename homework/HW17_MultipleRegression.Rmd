...
Most variables have some level of correlation. Bachelor's or higher and income are very strongly correlated, in addition to percent unemployed and turnount (though negative). Median age and bachelors or higher, however, do not have any correlation to note. Percent white and income are also less correlated. 

Population does not seem to have a linear relationship with the dependent variable. Percent white and unemployed seem to have linear relationships. The rest have weak or no relationships with the dependent variable. 
...
Overall, the model predicts turnout fairly well. It has a multiple r-squared of 0.8539, meaning 85.39% of the variance in turnout is explained by variables in this model. Some variables are indeed insignificant, such as population and percent bachelor's or higher. It does seem to be a useful model, as though some variables aren't significant the overall model still explains most of the change in the dependent variable. 
...
My model 2, which removed the bachelor's and population variables, vastly outperformed my other two and also slightly beat the full model. It has an adjusted r-squared of 0.8201642, an improvement of about 2% from the full model. 

```{r}
# Model 2
model2 <- lm(turnout ~ pct_unemployed + income + pct_white + median_age, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ income + median_age, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_white + pct_unemployed, data = md_turnout)
summary(model4)
...
I would put it into the simplest terms possible, focusing on the results rather than how we found them or why they are the result. For example, you could simply say that population size does not have an impact on voter turnout, and that percent of the population that have a bachelor's degree or higher doesn't have a significant impact, either. Overall, I would put it into terms of which variables have an impact on voter turnout numbers, and to what extent. 
